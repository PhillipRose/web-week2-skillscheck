...
const problem4 = sum = (num1, num2) => num1 + num2;
...
const problem5 = lovesCodeChecker = x => x === true ? true:false;
...
const problem6 = oddChecker = num => num % 2 === 1 ? 'the number is odd':'the number is even';
...
const problem7 = iLove = (name, love) => `${name} loves ${love}`;
...
const problem12 = bigOrSmall = arr => {
	let answers =[]
...
let problem13 = arrayReverser = arr => {
	let reversed = []
...
let doubled = []
let problem14 = myNumbers.map(elements => doubled.push(elements *= 2) )

//////////////////PROBLEM 15////////////////////

// Now, use .filter to create a new array containing any numbers that are greater than 100. Call the new array 'filtered'. 

// const problem15 = myNumbers.filter(function(ele, ind, arr15){
// 	let filtered = []
// 	if(ele > 100){
// 	filtered.push(ele)}
// })
let filtered = []
let problem15 = myNumbers.filter(function(ele){
	if (ele > 100){
		filtered.push(ele)
	}
})
...
// const finder = myNumbers.forEach = function(element, index, arr){
...
let problem18 = notGeorge.map(function(ele){
	if(ele !== 'George'){
...
let enemies = []
const problem19 = people.filter(function(arr){
  if (arr[`friend`] === false){
    enemies.push(arr)
  }
  return enemies
